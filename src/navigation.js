...
function BottomNavigation(params) {
    ...
    return <section id="bottom-navigation" className={`block fixed inset-x-0 bottom-0 z-10 bg-blue-700 shadow`}>
        <div id="tabs" className="flex justify-between text-white">
            <div onClick={() => navigate('/cars')} className={`w-full justify-center inline-block text-center pt-2 pb-1  ${location.pathname === '/cars' ? 'bg-blue-900' : ''} `}>
                <svg width="25" height="25" viewBox="0 0 42 42" className="inline-block mb-1 text-white">
            ...
            <div onClick={() => {sessionStorage.getItem('login') ? navigate('/booking') : navigate('/login')}} className={`w-full justify-center inline-block text-center pt-2 pb-1  ${location.pathname === '/booking' ? 'bg-blue-900' : ''} `}>
